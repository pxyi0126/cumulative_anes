    ...
    if pd.notna(var_2012):
        ...
        for v12 in var_2012_list:
            ...
            if sum2:
                ...
                sum2 += row12['description'].iloc[0] + '\n'
                q_2 += row12['description'].iloc[0] + '\n'
                if pd.notna(row12['valid_labels'].iloc[0]):
                    Valid2 += str(row12['valid_labels'].iloc[0]) + '\n'
                else:
                    Valid2 += '\n'
                if pd.notna(row12['missing_labels'].iloc[0]):
                    Missing2 += str(row12['missing_labels'].iloc[0]) + '\n'
                else:
                    Missing2 += '\n'
...
df_cdf.to_csv("/Users/yipho/anes/cumulative_anes/test/cdf_tester.csv", index=False, encoding = "utf-8-sig")
